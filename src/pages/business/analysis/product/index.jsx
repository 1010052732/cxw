...
export default function AnalysisProductPage() {
  ...
  useEffect(() => {
    ...
    if (q) {
      ...
      else if (/5g|电子|光纤|电路/i.test(decoded)) setProductName('电子产品')
      else if (/机床|机械|泵|自动/i.test(decoded)) setProductName('机械设备')
  ...
  const applyProduct = ({ catalogName, skuLabel: label, hs }) => {
  ...
  return (
    <div className="business-page">
      <div className="business-page-header">
        ...
        <p className="page-description">查询（列表选用）→ 价格 → 供需 → 壁垒 · 四维闭环 · 支撑采购、定价与合规决策</p>
      </div>
      ...
      <div className="business-filter-bar">
        <Space wrap>
          ...
          <Button size="small" icon={<OrderedListOutlined />} onClick={() => setBrowseOpen(true)}>
            从商品库选用
          </Button>
        </Space>
        <Space wrap>
      ...
      <ProductSearchModal
        ...
        onSelect={(row, nextFilters) => {
          ...
          applyProduct({ catalogName: row.parent || row.name, skuLabel: row.name, hs: row.hsCode })
        }}
      />
